angular.module('app').factory('catheterInfectService', catheterInfectService);

catheterInfectService.$inject = ['$http', '$q', 'SettingService'];

function catheterInfectService($http, $q, SettingService) {
    const service = {
        post,
        put,
        getAll,
    ...
    function getInfectionRecord(medicalId,Status = "") {
        ...
        $http({
            method: 'GET',
            url: `${serverApiUrl}/api/Infection_Record_All/GetInfectionRecord?Infection_Category=CATHETER&MedicalId=${medicalId}&Status=${Status}`
        }).then((res) => {
            deferred.resolve(res);
        }, (res) => {
            deferred.reject(res);
