const tpl = require('./allPatients.html');
require('./allPatients.less');

angular.module('app').component('allPatients', {
    template: tpl,
    controller: allPatientsController,
    controllerAs: 'vm'
});

allPatientsController.$inject = ['$localStorage', 'dialysisService', '$q', '$document', '$sce', '$rootScope', '$scope', '$interval', '$state', '$stateParams', '$mdSidenav', 'PatientService', 'SettingService', 'showMessage', '$timeout', 'nfcService', '$mdDialog', '$filter', 'SessionStorageService', 'userService'];

function allPatientsController($localStorage, dialysisService, $q, $document, $sce, $rootScope, $scope, $interval, $state, $stateParams, $mdSidenav, PatientService, SettingService, showMessage, $timeout, nfcService, $mdDialog, $filter, SessionStorageService, userService) {
    ...
    vm.$onInit = function () {
        ...
        console.log(SettingService.getCurrentUser());
        ...
        if ($state.current.name === 'allPatientsSearch') {
            ...
            PatientService.getByUserId(currentUser.Id, '')
                .then((d) => {
                    ...
                    allPatients = PatientService.orderPatients(d.data).filter((item) => {
                        return item.State != '2';
                    });
                    vm.loading = false;
                    vm.isError = false;
    ...
    vm.selectDoctor = function () {
        console.log('allPatients1', vm.old_patients);
        if (vm.sel_doctor != 'ALL') {
            allPatients = angular.copy(vm.old_patients);
            allPatients = allPatients.filter((item) => {
                return item.AttendingPhysician == vm.sel_doctor;
            });
            vm.currentPatients = allPatients;
            vm.patients = allPatients;
            console.log('allPatients3',vm.currentPatients,vm.patients);
        }
        else {
            getAllPatients();
        }
    ...
    function keypress(ev) {
        if (ev.which === 13) {
            ...
            $timeout(() => {
                ...
                if ($state.current.name !== 'allPatientsSearch' && $state.current.name !== 'summary' && $('#search').val() !== '') {
                    $state.go('allPatientsSearch', { str: $('#search').val(), isScanBarcode: true }, {
            ...
            $timeout(() => {
                if ($state.current.name !== 'allPatientsSearch') {
                    $state.go('allPatientsSearch', { str: $('#search').val(), isScanBarcode: false }, {
                        notify: false, reload: false, location: 'replace'
                    });
                }
            }, 500);
    ...
    vm.searchPatient = function (searchStr) {
        ...
        $state.go('allPatientsSearch', { str: vm.search, isScanBarcode: barcodeBool }, {
            notify: false, reload: false, location: 'replace'
        }).then(() => {
            ...
            if (searchStr) {
                PatientService.searchPatient(currentUser.Id, searchStr)
                    .then((res) => {
                        // 依特定條件排序 patients: 依剩餘時間排序，null代表尚未洗排最上面; 若都沒有剩餘時間已後修改的為大
                        vm.patients = PatientService.orderPatients(res.data).filter((item) => {
                            return item.State != '2';
                        });
        ...
        PatientService.getByUserId(currentUser.Id, isForce, '')
            .then((d) => {
                ...
                if ($state.current.name === 'allPatients') {
                    vm.patients = angular.copy(allPatients).filter((item) => {
                        return item.State != '2';
                    });
    ...
    vm.goToDialysis = function (item) {
        vm.loading = true;
        // TODO: check for bugs
        SessionStorageService.setItem('homeStateName', 'allPatients'); // 紀錄由病人清單到summary，由summary返回上一頁時要回到病人清單
        $state.go('summary', { patientId: item.Id }, { notify: true, reload: true, inherit: false });
    };

    vm.goToApd = function (item) {
        vm.loading = true;
        // TODO: check for bugs

        $state.go('apdSetting', { patientId: item.Id });
        //$state.go("apdSetting", {patientId: vm.currentPatients[0].Id});
